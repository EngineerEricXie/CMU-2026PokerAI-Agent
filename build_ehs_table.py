...
def process_batch(batch):
    ...
    for H, C in batch:
        ...
        for O in itertools.combinations(unknown_cards, 2):
            opp_curr_score = curr_dict[tuple(sorted(O + C))]
            
            if hero_curr_score < opp_curr_score:
                curr_idx = 0 
            elif hero_curr_score == opp_curr_score:
                curr_idx = 1 
            else:
                curr_idx = 2 
                
            totals[curr_idx] += 1
            rem_deck = [c for c in unknown_cards if c not in O]
            board_needed = 2 if is_flop else 1
            
            for F in itertools.combinations(rem_deck, board_needed):
                hero_final_score = L7[tuple(sorted(HC + F))]
                opp_final_score = L7[tuple(sorted(list(O) + list(C) + list(F)))]
        ...
        # --- 計算指標 ---
        total_opp_hands = sum(totals)
        HS = (totals[0] + 0.5 * totals[1]) / total_opp_hands
        
        denom_ppot = totals[2] + 0.5 * totals[1]
        PPot = (HP[2][0] + 0.5 * HP[2][1] + 0.5 * HP[1][0]) / denom_ppot if denom_ppot > 0 else 0.0
            
        denom_npot = totals[0] + 0.5 * totals[1]
        NPot = (HP[0][2] + 0.5 * HP[0][1] + 0.5 * HP[1][2]) / denom_npot if denom_npot > 0 else 0.0
