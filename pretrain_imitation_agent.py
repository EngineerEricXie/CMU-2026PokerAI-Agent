...
# python pretrain_imitation_agent.py

# # 用 ProbabilityAgent 當老師
# python pretrain_imitation_agent.py --teacher probability --episodes 5000

# # 用 V10 當老師（最強但最慢）
# python pretrain_imitation_agent.py --teacher v10_old --episodes 3000 --epochs 30

# # 指定輸出路徑（例如存成另一個檔案）
# python pretrain_imitation_agent.py --teacher v10_old --output agents/v10_pretrain_weights.pth
...
def collect_demonstrations(num_episodes=20000, teacher="fast"):
    """
    讓兩個指定的老師互打，收集 (state, action) 資料。
    對雙方都收集，資料量加倍。
    """
    env = PokerEnv()

    states        = []   # List[np.array(16,)]
    action_types  = []   # List[int]  0-4
    raise_idxs    = []   # List[int]  0-99  (raise amount - 1)
    discard_idxs  = []   # List[int]  0-9

    print(f"Collecting {num_episodes} episodes using '{teacher}' teacher...")
