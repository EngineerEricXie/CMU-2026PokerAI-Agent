...
def collect_demonstrations(num_episodes=20000, player_id=0):
    """
    讓兩個 ProbabilityAgent 互打，收集 player_id 的 (state, action) 資料。
    對雙方都收集，資料量加倍。
    """
    env     = PokerEnv()

    states        = []   # List[np.array(16,)] - 用 numpy 省記憶體，最後再轉 tensor
    action_types  = []   # List[int]  0-4
    raise_idxs    = []   # List[int]  0-99  (raise amount - 1)
    discard_idxs  = []   # List[int]  0-9

    print(f"Collecting {num_episodes} episodes (fast teacher, 30 sims)...")

    for ep in range(num_episodes):
        if (ep + 1) % 2000 == 0:
            print(f"  {ep+1}/{num_episodes} episodes, {len(states)} transitions")
        ...
        while not done:
            ...
            # 雙方都用 fast_teacher_act，雙方資料都收集
            if acting == 0:
                teacher_obs = obs[0]
                teacher_act = fast_teacher_act(obs[0], _EVALUATOR)
            else:
                teacher_obs = obs[1]
                teacher_act = fast_teacher_act(obs[1], _EVALUATOR)

            state = preprocess_observation(teacher_obs).numpy()  # 存 numpy 省記憶體
            act_type, raise_amt, keep1, keep2 = teacher_act

            # raise index：ProbabilityAgent 給的是實際金額，轉回 0-99 index
            raise_idx = 0
            if act_type == PokerEnv.ActionType.RAISE.value and raise_amt > 0:
                raise_idx = max(0, min(99, raise_amt - 1))

            # discard index：(keep1, keep2) → KEEP_PAIR_TO_IDX
            discard_idx = 0
    ...
    print(f"Collection done: {len(states)} transitions from {num_episodes} episodes")

    # 各動作佔比
    from collections import Counter
    cnt = Counter(action_types)
    ...
    # 一次性從 numpy 轉成 tensor，比逐個 stack 更省記憶體
    states_np = np.array(states, dtype=np.float32)
    return (torch.from_numpy(states_np),
            torch.tensor(action_types, dtype=torch.long),
            torch.tensor(raise_idxs,   dtype=torch.long),
            torch.tensor(discard_idxs, dtype=torch.long))

# ── 預訓練 ────────────────────────────────────────────────────────────────

def pretrain(num_episodes=20000, epochs=30, batch_size=512,
             lr=1e-3, weight_path=None):
    ...
    states, action_types, raise_idxs, discard_idxs = \
        collect_demonstrations(num_episodes)
...
if __name__ == "__main__":
    pretrain(
        num_episodes=10000,   # 雙方都收集約 4~5 萬 transitions，夠用
        epochs=50,
        batch_size=512,
        lr=1e-3,
    )