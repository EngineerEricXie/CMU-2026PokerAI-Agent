# ==========================================
# RLAgent V3.1 - 混合神经引擎版本 (EHS + RL) + timer
# ==========================================
# 1. 优化了弃牌loop的速度
# 2. 加上了timer控制时间
...
# --------超时备用-----------------
def fast_compute_equity(obs):
    """
    【极速版】特征胜率提取，防止给神经网络喂数据时卡死
    """
    street = obs.get("street", 0)
    my_cards_raw = valid_cards(obs["my_cards"])
    community_cards = valid_cards(obs["community_cards"])
    opp_discarded = valid_cards(obs.get("opp_discarded_cards", [-1, -1, -1]))
    ...
    if len(my_cards) != 2 or GLOBAL_LOOKUP_TABLE is None: return 0.5
    ...
    wins = ties = total = 0
    comm_tup = tuple(community_cards)
    my_tup = tuple(my_cards)

    # 🚀 同样使用循环倒置提速
    for future_comm in itertools.combinations(unknown_cards, board_needed):
        board_5 = comm_tup + future_comm
        my_score = GLOBAL_LOOKUP_TABLE[tuple(sorted(my_tup + board_5))]
        
        rem_deck = [c for c in unknown_cards if c not in future_comm]
        for opp_2 in itertools.combinations(rem_deck, 2):
            opp_score = GLOBAL_LOOKUP_TABLE[tuple(sorted(opp_2 + board_5))]
...
def fast_preprocess_observation(obs):
    ...
    equity = np.array([fast_compute_equity(obs)])
...
class RLAgentV3(Agent):
    ...
    def act(self, observation, reward, terminated, truncated, info):
        ...
        # 🧠 第四层博弈：RL 神经网络接管下注博弈
        # --------------------------------------------------------
        if self.act_accumulated_time < 1300.0:
            # ✈️ 【主引擎】常规状态：使用原版最熟悉的特征提取
            state = preprocess_observation(observation).to(self.device)
        else:
            # 🚀 【备用极速引擎】危机状态：强制断尾求生！
            self.logger.warning(f"⚠️ 算力时间警报！已耗时 {self.act_accumulated_time:.1f}s，强制启动极速降级引擎！")
            state = fast_preprocess_observation(observation).to(self.device)
        ...
        self.act_accumulated_time += (act_end_time - act_start_time)
